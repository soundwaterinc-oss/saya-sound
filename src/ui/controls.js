  ...
  function render() {
    ...
    if (state.geo.source === 'procedural') {
      src.append(
        ...
        select(L.hand, [['卍', state.lang === 'ja' ? '卍 一様' : '卍 uniform'], ['逆', '逆'], ['checker', L.checker]], state.geo.handedness, (v) => set('geo.handedness', v, cb.regen)),
        slider(L.armLen, 0.3, 1.1, 0.01, state.geo.armLen, (v) => set('geo.armLen', v, cb.regen)),
